...
if((isset($_GET['startDate']) && $_GET['startDate'] != null) && (isset($_GET['endDate']) && $_GET['endDate'] != null)){
    ...
    if( !is_null($business) && $business != '' ){
        $sql = $standardQuery . ' WHERE start_datetime >= "'.$startDate.'" AND start_datetime <= "'.$endDate.'" AND cl.id = '.$businessId.' GROUP BY em_a.id ORDER BY start_datetime DESC';

    }else {
        $sql = $standardQuery . ' WHERE start_datetime >= "'.$startDate.'" AND start_datetime <= "'.$endDate.'" GROUP BY em_a.id ORDER BY start_datetime DESC'; 
    }
...
while ($data = $employeeAccess->fetch_assoc()){ 
    ...
    $file .= '<td>'.utf8_decode( $data['driver_vehicle'] ).'</td>';
